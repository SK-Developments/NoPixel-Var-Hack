    ...
    if (game_started && valid_keys.includes(key_pressed)) {
        switch (key_pressed) {
            case 'Escape':
                ...
                destroy()
                $.post('https://var-hack/callback', JSON.stringify({ 'success': false }));
    ...
    if (parseInt(ev.target.dataset.number, 10) === order) {
        ...
        if(order > numbers){
            ...
            setTimeout(function() { 
                ...
                destroy()
                $.post('https://var-hack/callback', JSON.stringify({ 'success': true }));
            }, 2000);
        ...
        document.querySelector('.splash .text').innerHTML = 'SEQUENCE FAILED!';
        //document.querySelector('.splash').classList.remove('hidden');
